...
export class lunar
{
    ...
    private solarToInt(y: number, m: number, d: number)
    {
        ...
        d = parseInt(d.toString(), 10);

        m = (m + 9) % 12;
        y = y - parseInt((m / 10).toString());
    ...
    public solar(isLeanMonth?: boolean)
    {
        ...
        let days = this.lunarMonthday[this.year() - this.lunarMonthday[0]];
        let leap = this.getBitInt(days, 4, 13);
        let offset = 0;
        let loopend = leap;
        ...
        let solar11 = this.solarYear[this.year() - this.solarYear[0]];

        let y = this.getBitInt(solar11, 12, 9);
        let m = this.getBitInt(solar11, 4, 5);
        let d = this.getBitInt(solar11, 5, 0);

        let g = this.solarToInt(y, m, d) + offset - 1;

        y = parseInt(((10000 * g + 14780) / 3652425).toString());
        let ddd = g - (365 * y + parseInt((y / 4).toString()) - parseInt((y / 100).toString()) + parseInt((y / 400).toString()));
        if (ddd < 0)
        {
            y--;
            ddd = g - (365 * y + parseInt((y / 4).toString()) - parseInt((y / 100).toString()) + parseInt((y / 400).toString()));
        }
        let mi = parseInt(((100 * ddd + 52) / 3060).toString());
        let mm = (mi + 2) % 12 + 1;
        y = y + parseInt(((mi + 2) / 12).toString());
        let dd = ddd - parseInt(((mi * 306 + 5) / 10).toString()) + 1;
    ...
    public lunar()
    {
        ...
        let data = (this.year() << 9) | ((this.month() + 1) << 5)
            | (this.date());
        if (this.solarYear[index] > data)
        {
            index--;
        }
        let solar11 = this.solarYear[index];
        let y = this.getBitInt(solar11, 12, 9);
        let m = this.getBitInt(solar11, 4, 5);
        let d = this.getBitInt(solar11, 5, 0);
        let offset = this.solarToInt(this.year(), this.month() + 1,
            this.date()) - this.solarToInt(y, m, d);

        let days = this.lunarMonthday[index];
        let leap = this.getBitInt(days, 4, 13);

        let lunarY = index + this.solarYear[0];
        let lunarM = 0;
        offset += 1;

        for (let i = 0; i < 13; i++)
        {
            let dm = this.getBitInt(days, 1, 12 - i) == 1 ? 30 : 29;
            if (offset > dm)
            {
                lunarM++;
                offset -= dm;
        ...
        let lunarD = parseInt(offset.toString());
        ...
        return new Date(lunarY, lunarM, lunarD);
    }
