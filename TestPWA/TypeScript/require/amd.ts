...
(function (root)
{
	...
	let that = {
		 ...
		 */
		require: function (dependencyNames:string[], callback:Function, from:string)
		{
			...
			// Get dependencies
			let dependencies = that.resolveDependencies(dependencyNames, from);
		 ...
		 */
		define: function (name:string, dependencies:string[], callback:Function)
		{
		 ...
		 */
		normalizeModuleName: function (name:string, baseName:string)
		{
		...
		resolveDependencies: function (dependencyNames:string[], from:string)
		{
			...
			return dependencyNames.map(function (dependencyName)
			{
				...
				let dependencyPath = that.normalizeModuleName(dependencyName, from);

				// Get dependency instance
				let dependencyInstance = that.resolveDependency(dependencyPath);
		 ...
		 */
		resolveDependency: function (dependencyPath:string)
		{
			...
			else if (dependencyPath in that.globalDependencies)
			{
				...
				let dependencyGlobalName = that.globalDependencies[dependencyPath];
			...
			else if (dependencyPath in that.waitingModules)
			{
				...
				let moduleToBuild = that.waitingModules[dependencyPath];

				// Resolve waiting module dependencies recursively
				let dependencies = that.resolveDependencies(
