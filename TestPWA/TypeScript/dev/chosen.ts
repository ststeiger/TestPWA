...
declare let previousSelectedValue:any;
...
export class Chosen
{
	...
	protected items: any;
	protected currentItems: any;


	protected htmlComponents: Element[];
	protected resultsElements: any[];
	protected query: string;
	protected $selectedResult: any;

	protected $dropdownContainer: any;
	protected $dropdownMask: any;
	...
	public buildHtml(items:any, $element:HTMLSelectElement, options:IChosenOptions)
	{
	...
	public buildDropdown(options:IChosenOptions, items:any)
	{
	...
	public buildResults(items:any)
	{
		...
		let htmlComponents = this.htmlComponents,
			resultsElements = this.resultsElements,
			options = this.options,
			query = this.query,
			selectedValue = this.selectedValue;

		return items.map(function (item, index:number)
		{
	...
	public getItems($select:HTMLSelectElement)
	{
	...
	public getItemByValue(value:any)
	{
		value = value || null;

		let matches = this.items.filter(function (item:any)
		{
	...
	public appendResults($results:any, $resultsContainer:any)
	{
	...
	public queryItems(query: any, selectedValue: any)
	{
		let queryResults;
		...
		{
			queryResults = this.items.filter(function (item: any)
			{
		...
		if (selectedValue)
		{
			...
			let indexOfSelectedValue = queryResults.map(function (result: any)
			{
				return result.value;
