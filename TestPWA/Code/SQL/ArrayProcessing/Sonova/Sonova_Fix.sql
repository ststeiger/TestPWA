...
(
	SELECT 
		...
		,T_AP_Kommunikation.KM_ZN_UID 
		,T_AP_Kommunikation.KM_Nr 
		,T_AP_Kommunikation.KM_Rufnummer
		,LOWER(drawing_km_uid) AS drawing_km_uid 
		,LOWER(drawing_zn_uid) AS drawing_zn_uid 
		 
		,'checkKeypair("' + LOWER(drawing_km_uid) + '", "' + LOWER(drawing_zn_uid) + '");' AS checkCommand 
	FROM T_AP_Kommunikation 

	INNER JOIN 
		(
			SELECT 
				 tKeyPairs.drawing_km_uid 
				,tKeyPairs.drawing_zn_uid 
			FROM OPENJSON(@json) WITH (drawing_km_uid uniqueidentifier '$.km_uid', drawing_zn_uid uniqueidentifier '$.zn_uid') AS tKeyPairs 
		) AS t 
		ON t.drawing_km_uid = T_AP_Kommunikation.KM_UID 
) 
,CTE AS 
(
	SELECT * FROM BaseData 
	WHERE (1=1) 
	-- AND (KM_ZN_UID <> drawing_zn_uid) 
	-- AND KM_ZN_UID IS NULL AND drawing_zn_uid IS NOT NULL -- 6BFD64F8-E12A-4020-888F-D7331EF1585D
	-- AND KM_ZN_UID IS NOT NULL AND drawing_zn_uid IS NULL 

	AND 
	(
		(KM_ZN_UID <> drawing_zn_uid) 
		OR 
		(KM_ZN_UID IS NULL AND drawing_zn_uid IS NOT NULL) 
		OR 
		(KM_ZN_UID IS NOT NULL AND drawing_zn_uid IS NULL) 
	) 
)
SELECT * FROM CTE 
-- UPDATE CTE SET KM_ZN_UID = drawing_zn_uid
