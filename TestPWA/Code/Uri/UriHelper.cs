    ...
    internal static class UriHelper
    {
            ...
            bool isUriString, char force1, char force2, char rsvd)
        {
            if (end - start >= Uri.c_MaxUriBufferSize)
                throw new UriFormatException("Size Limit");
                    ...
                    if (ch > '\x7F')
                    {
                        ...
                        if (pStr[i + count - 1] >= 0xD800 && pStr[i + count - 1] <= 0xDBFF)
                        {
                            ...
                            if (count == 1 || count == end - i)
                                throw new UriFormatException("Bad String");
                    ...
                    while (true)
                    {
                        ...
                        if (allBytesMatch)
                        {
                            ...
                            if (iriParsing)
                            {
                                ...
                                else if (!Uri.IsBidiControlCharacter(unescapedCharsPtr[j]) || !UriParser2.DontKeepUnicodeBidiFormattingCharacters)
                                {
                                    //copy chars
                                    Debug.Assert(dest.Length > destOffset, "Buffer overrun detected");
        ...
        internal static bool IsNotSafeForUnescape(char ch)
        {
            ...
            else if (UriParser2.DontEnableStrictRFC3986ReservedCharacterSets)
            {
        ...
        private static unsafe bool IsReservedUnreservedOrHash(char c)
        {
            ...
            if (UriParser2.ShouldUseLegacyV2Quirks)
            {
