...
var lunar = (function () {
    ...
    lunar.prototype.solar = function (isLeanMonth) {
        isLeanMonth = isLeanMonth === true;
        var days = this.lunarMonthday[this.year() - this.lunarMonthday[0]];
    ...
    lunar.prototype.lunar = function () {
        var index = this.year() - this.solarYear[0];
        var data = (this.year() << 9) | ((this.month() + 1) << 5)
            | (this.date());
        if (this.solarYear[index] > data) {
            index--;
        }
        var solar11 = this.solarYear[index];
        var y = this.getBitInt(solar11, 12, 9);
        var m = this.getBitInt(solar11, 4, 5);
        var d = this.getBitInt(solar11, 5, 0);
        var offset = this.solarToInt(this.year(), this.month() + 1, this.date()) - this.solarToInt(y, m, d);
        var days = this.lunarMonthday[index];
        var leap = this.getBitInt(days, 4, 13);
        var lunarY = index + this.solarYear[0];
        var lunarM = 0;
        offset += 1;
        for (var i = 0; i < 13; i++) {
